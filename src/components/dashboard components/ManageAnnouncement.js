...
import { Jumbotron, Row, Col, Button, ButtonGroup, Spinner, Container, Modal } from 'react-bootstrap'
...
function deleteAnnouncement(item){
    ...
    db.doc(item.id).delete().then(()=>{
        setMessage({type:'primary', msg:'Successfully deleted announcement'})
    }).catch(()=>{
...
function DeleteModal(){
    ...
    return(
        <Modal show={showModal} onHide={handleCloseModal}>
            ...
            <Modal.Footer>
            <ButtonGroup>
                <Button variant="primary" size="sm" className="w-75 m-3" onClick={()=>{deleteAnnouncement(selectedItem)}}>
                    Yes
                </Button>
...
function MessageModal(){
    ...
    if(message == null|| typeof message == undefined){
        ...
        return(
            ...
            <Modal.Footer>
            <ButtonGroup>
            <Button variant="primary" size="sm" className="w-75 m-3" onClick={()=>{setMessageShowModal(false);getData()}}>
                   Close
            </Button>
            </ButtonGroup>
            </Modal.Footer>
...
function RenderAnnouncements(){
    if(announcement.length === 0) return(<div style={{display:'flex', justifyContent:'center'}}>
        <p>No Announcements</p>
    </div>)

let filtered = announcement.filter(item => item.author === currentUser.email)
    ...
    return(
        filtered.map((item)=>{
                    ...
                    return(
                        ...
                        <Col lg={3}>
                        <Row>
                            <Button variant="danger" size="sm" className="m-auto" onClick={()=>{setSelectedItem(item);setShowModal(true)}}>
                                Delete
                            </Button>
                        </Row>
                        </Col>
