...
import { Col, Container, Image, Jumbotron, Row, Spinner } from 'react-bootstrap'
...
function NewsAndEvents (){
                ...
                return(
                    ...
                    <Col lg={10} className="w-100 border">
                        ...
                        <Row lg={4} className="w-100">
                            <h3 style={{textDecoration:'underline', textDecorationColor:colours[index]}}>
                                {type.label}
                            </h3>
                        </Row>
    ...
    function RenderPosts({type}){
        ...
        return filtered.map((post, index)=>{
                    return(<Col md={6} lg={4} xs={12} key={post.id} className="border">

                                {index === 0
                                ?<div>
                                <Image src={post.url} width="100%" height="100%"/>
                                    <div id="postDesc">
                                        <Row lg={12} className="w-100"><p style={{color:'white'}}>{post.title}</p></Row>
                                        <Row lg={12} className="w-100"><p style={{color:'white', fontSize:10}}>{post.subtitle}</p></Row>                                                          
                                    </div>
                                </div>
                                :null}
